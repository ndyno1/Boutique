...
// Adaptation Apps Script:
// - get_products renvoie: c (col C), k (col K)
const fromK = (p) => safe(p?.k ?? p?.K).trim(); // page produit
const fromC = (p) => safe(p?.c ?? p?.C).trim(); // page paiement
...
function buildPayUrl(p) {
  // Paiement: on envoie seulement desc = colonne C
  ...
  // ✅ IMPORTANT: paiement affiche seulement C
  qp.set("desc", fromC(p));

  // optionnel (si paiement utilise img)
...
function templateProductPage(p) {
  ...
  // ✅ Page produit = colonne K
  const descK = fromK(p);

  // URL paiement: contient desc=C (dans buildPayUrl), qty sera ajouté côté JS
...
<script>
  ...
  const maxQ = parseInt(PRODUCT.max) || 0;
  ...
  function shareProduct() {
    ...
    if (navigator.share) {
      navigator.share({ title: PRODUCT.nom, text: 'Regarde ce produit sur ViralFlowr !', url });
    } else {
