...
// NEW: pick non-empty field from multiple keys
const pick = (obj, keys) => {
  for (const k of keys) {
...
function templateProductPage(p) {
  ...
  // description fields (support many keys)
  const desc = pick(p, [
    "desc", "description", "instructions", "note", "info",
    "DESC", "DESCRIPTION", "INSTRUCTIONS"
  ]);

  const longDesc = pick(p, [
    "long_desc", "long", "longDescription", "details", "detail",
    "LONG_DESC", "LONG", "DETAILS"
  ]);
  ...
  // build payment base url (we will append qty in JS)
  const payBase = buildPayUrl({
    ...p,
    id, nom, cat,
    prix: prixNum,
    desc,
  ...
  const ogDesc = `${prixNum} $ • ${cat}${(longDesc || desc) ? " • " + (longDesc || desc).replace(/\s+/g, " ").slice(0, 140) : ""}`.slice(0, 200);
  ...
  // === STYLE EXACT DU SITE (TON TEMPLATE) + données injectées ===
  return `<!DOCTYPE html>
...
<body class="flex flex-col min-h-screen">
    ...
    <script>
      ...
      // Initialisation UI (sans URL params)
      ...
      const descText = (PRODUCT.longDesc || PRODUCT.desc || "Livraison instantanée par email après confirmation du paiement. Code officiel garanti.");
      document.getElementById('product-desc').textContent = descText;
      ...
      function shareProduct() {
        ...
        if (navigator.share) {
          navigator.share({
            title: productNom,
            text: 'Regarde ce produit sur ViralFlowr !',
            url,
          });
        } else {
