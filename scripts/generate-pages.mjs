...
function renderProductPage(prod) {
  const id = prod.id;
  const canonical = `${SITE_BASE}/p${encodeURIComponent(id)}.html`;
...
<head>
  ...
  <style>
    :root{ --vf-orange:#F07E13; --vf-bg:#F3F3F3; --vf-text:#201B16; }
  ...
  <main class="max-w-[900px] mx-auto px-4 py-10">
    <div class="bg-white shadow-sm rounded-xl p-6 md:p-8">
      <div class="text-sm leading-relaxed whitespace-pre-line break-words">
        ${linkify(escapeHtml(prod.long_desc || "Aucune description disponible."))}
      </div>
    </div>
  </main>
</body>
