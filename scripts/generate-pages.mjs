    ...
    <div class="max-w-[1240px] mx-auto px-4 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
      <div class="bg-white shadow-sm rounded-xl p-6 md:p-8">
        ...
        <div class="flex flex-col sm:flex-row gap-6">
          <img src="${escapeAttr(ogImg)}" class="w-32 h-32 object-contain rounded-xl border p-2 bg-white" alt="${escapeAttr(prod.nom)}">
          <div class="text-sm leading-relaxed whitespace-pre-line break-words">
            ${linkify(escapeHtml(prod.long_desc || prod.desc || "Aucune description disponible."))}
          </div>
        </div>
      </div>
...
async function main() {
  ...
  for (const raw of rawList) {
    ...
    if (!prod.id) continue;
    if (EXPECT_ID && prod.id !== EXPECT_ID) continue;
